...
- **Structured re-render (CE Health, CVR-RCA).** CE Health and CVR-RCA both emit
  *deterministic structured data* (a JSON sidecar + regular GFM tables) that we
  ...
  mis-specified, so the tab shows a corrected canonical 5-factor booking-revenue
...
> **All money is USD — always render the `$` symbol, never localize.** Every money
> value in this preview (Revenue, AOV, and the Shapley `$` contributions) is **USD** —
> the engine reads `*_usd` columns. The CE's **market / country pills (e.g. "London ·
> UK", "Paris · Europe") describe the CE's *location*, NOT its currency** — do **NOT**
> swap in £ / € / ₹ / ¥ for a non-US CE. A UK CE's AOV is `$264`, not `£264`. (The
> rendered CE Health tab already hardcodes `$`; this note keeps the in-chat preview
> consistent.)
...
*Shapley drivers reconcile to total revenue, calculated **including PMax** (all
channels). The Shapley **CVR = orders ÷ users** (it absorbs orders-per-converter — no
separate driver), so it differs from the Vitals CVR row above (converters ÷ users,
Omni / PMax-excluded); the Vitals also exclude PMax.*
...
- **header:** "Your read"
- **question** — **plain language, no jargon** (don't dress it up — say it the way you'd
  ask a colleague; avoid phrases like "scalable lever you want sized"). One tight line +
  one short example: *"Given the numbers — what do you think is driving this, and where
  should I dig first?"* e.g. *"the SIS pullback is the story — look at which campaigns
  it hit" · "the CVR lift — was it the B1G1 promo or the LP change?"*
...
- **perf-audit** — **must NOT run its Step-1 date self-computation** (which would
  ...
  **Step 4b is MANDATORY — name it explicitly so it is not skipped.** perf-audit's
  Step 4b (Create Google Sheet — full 8-tab data dump, link appended to its §B Data
  Sources) sits *after* the report narrative reads as finished, so a sub-agent tends
  to stop at the written report and skip it — leaving the report's "full dump in Tab
  6/7/8" pointers dangling (an observed failure). In the dispatch prompt, instruct
  perf-audit explicitly: **"The run is NOT complete until Step 4b has run and Section
  B of `perf_audit_report.md` contains either the data-dump Sheet link or the local
  CSV fallback paths — do not stop at the written report."** This is always
  satisfiable (Sheet link *or* the graceful local-CSV fallback), so it never blocks.
...
**Post-dispatch gate — verify perf-audit's data dump landed (only if perf-audit ran).**
The dispatch prompt above marks Step 4b mandatory, but verify rather than trust — it
is the step most prone to being skipped. Once perf-audit reports done, check that its
§B carries the data dump:

```bash
# Section B should contain a Sheet link OR local CSV fallback paths
awk '/^## B\. Data Sources/{f=1} f' "<run_dir>/perf_audit_report.md" \
  | grep -qiE 'data dump|spreadsheet|docs\.google\.com|\.csv|Tab 1:' \
  && echo "perf §B: data dump present" || echo "perf §B: MISSING — Step 4b skipped"
```

If it prints `MISSING`, **re-prompt the same perf-audit agent** (via `SendMessage` to
its agent id — its context is intact) to *"run Step 4b only: create the 8-tab data-dump
Sheet (or the local-CSV fallback) and append the link/paths to §B of
`perf_audit_report.md`. Do not redo anything else."* Re-check, then proceed. Log the
outcome in `logs/_run_log.md`. (Graceful: if it still can't produce it after one
re-prompt — e.g. no Drive access and CSV export fails — note it and continue; the §B
pointers stay dangling but the run isn't blocked.)
...
the corrected 5-factor Shapley waterfall. (Absent insights → deterministic callouts;
...
| # | Date | Changes |
| --- | --- | --- |
| m099 | 2026-07-09 | **Harden the gcloud installer download — no stale/planted `/tmp` file (v2.60.2).** Review (github agent) flagged a TOCTOU in `scripts/onboarding.sh`: the SDK installer was fetched to a **fixed, world-writable** path `/tmp/gcloud_install.sh`, and the bundled-Python fallback reused a pre-existing one (`[ -f … ] || curl`) — so a stale file (interrupted run) or one planted by another local user could be executed as-is. Both install blocks now `mktemp` a unique temp file, download → execute that, then delete it. No normal-run behavior change. Consciously **skipped** the review's other asks (same-repo ZIP checksum = theater vs the stated threat; GPG signing = overkill for an internal tool; moving off the personal GitHub repo = a separate hosting decision) — distribution stays on the personal repo per the owner. Blast radius: `scripts/onboarding.sh` (2 blocks), `VERSION`, `CHANGELOG.md`, `SKILL.md` (this row). No engine / renderer / contract change; plugin copy untouched. |
| m098 | 2026-06-23 | **Slash commands register from one source of truth — new commands now propagate on update (v2.60.0).** Fixes the gap where v2.59.0's `/ce-rca-drive-sync` shipped in the bundle but never registered for existing users (per-run auto-update only re-downloaded files; the update path checked only the old commands). New **`scripts/register_commands.sh`** writes every `~/.claude/commands/*.md` (idempotent) and is now called at **install** (INSTALL Step 3 replaces its 60-line inline block with one call), on **explicit update** (INSTALL § Update U2, unconditionally), and by the **per-run auto-update** (`update_guard.sh` after a successful download + the umbrella's in-run update block). Add a command there once → it lands on every user's next run automatically. Blast radius: **new** `scripts/register_commands.sh`; `INSTALL.md`, `scripts/update_guard.sh`, `SKILL.md` (umbrella update block + this row), `VERSION`, `CHANGELOG.md`. No umbrella-flow / renderer / contract change; plugin copy untouched. |
| m097 | 2026-06-23 | **New sub-skill `/ce-rca-drive-sync` — back-fill runs to Drive + collect feedback (v2.59.0).** A durable, on-demand maintenance skill for two gaps after v2.58.0 enforced archival: pre-v2.58.0 runs that never synced, and no feedback ever collected. **New `skills/ce-rca-drive-sync/SKILL.md`** (`disable-model-invocation`, explicit-only): **Phase 1** runs **new `scripts/drive_backfill.py`** — sweeps `~/Documents/CE RCA Runs/` (env `CE_RCA_RUNS_DIR`), checks Drive authoritatively per run (re-added `drive_sync.find_existing_folder()`), and for any run with a `report.html` not on Drive writes a `reason.md` (inferred from `logs/_run_log.md`), archives it via `auto_archive()`, uploads `reason.md`, writes `_backfill_summary.md` (`--dry-run` read-only; classify-only if Drive unset; never deletes). **Phase 2** walks runs lacking `feedback.md` newest-first, one at a time (recall card → reply → append + upload `feedback.md`). Idempotent/safe to re-run. Registered as a 6th command in `INSTALL.md` Step 3 + Step-5 brief + README. Blast radius: **new** `skills/ce-rca-drive-sync/SKILL.md` + `scripts/drive_backfill.py`; `scripts/drive_sync.py` (`find_existing_folder`), `INSTALL.md`, `README.md`, `VERSION`, `CHANGELOG.md`, `SKILL.md` (this row). No umbrella-flow / renderer / contract change; plugin copy untouched. |
...
| m084 | 2026-06-16 | **CE Health §4 funnel-by-dimension: YoY deltas for Channel & Language, matching Landing Pages (v2.49.0).** In the §4 "Funnel by dimension" dropdown, the **Landing Pages** panel carried a YoY comparison but **Channel** and **Language** showed current-period values only — no deltas — because the engine pulled just the current window for those cuts (a stakeholder asked for at least LP Users + Site CVR deltas there). Not solvable by a report-time prompt: the comparison data was never queried. Fix (engine-side; `vendor.sh` disabled → `skills/` canonical, no re-vendor): **(1)** `fetch_funnel_by_dimension(ce_id, cur, ly)` now pulls the last-year same-period window too (optional `ly` arg — backward compatible, current-only when omitted) and folds `ly_<metric>` keys (LP Users / LP2S / S2C / C2O / S2O / Site CVR) onto each current row, matched by dimension value — the same YoY basis the Landing-Pages cut uses. **(2)** `render_funnel_by_dimension` emits an inline YoY delta per metric (counts → % change via `dp`, rates → pp via `dpp`) which the renderer's existing `split_deltas` turns into a value + coloured badge — identical to the §4 CE funnel + Landing-Pages cut; new channels/languages with no prior-year row render value-only (no fabricated delta). **(3)** call site passes `ly_cur`. **No `render_ce_health.py` change** — those panels already passed `split_deltas=True`; they just had nothing to split. Verified on CE 2567: Channel + Language panels now show per-row YoY deltas (e.g. Google Ads LP Users +223%, Site CVR +0.7pp) and the renderer parses each cell into a value + badge. Blast radius: `skills/ce-health/engine/sources/bq.py`, `skills/ce-health/ce_health.py` (render + call site), `SKILL.md` (this row), `CHANGELOG.md`, `VERSION`. No renderer / `compose.py` / template / CVR-RCA / perf-audit / contract change. |
| m083 | 2026-06-16 | **Shapley drops "Orders / Converter" — CVR absorbs it — consistently across all three surfaces (v2.48.0).** The Shapley carried a separate **orders-per-converter** factor (orders ÷ converting-users) that confused stakeholders and was artifact-prone (its funnel converter denominator is a different identity than the order numerator → inflated at peak volume). Removed: the Shapley **CVR is redefined as orders ÷ users**, folding the users→converters→orders steps into one factor — no converter count, no leaky denominator. Identity stays exact, telescopes to actual net revenue (`traffic × (O/U) × (GB/O) × (GBC/GB) × (NET/GBC) = NET`). Applied **identically** on every surface a Shapley appears so they never disagree: **(1)** the orchestrator Step-1 write-up (engine `compute_shapley_for_ce`, recast by the user), **(2)** the **§7 Shapley waterfall chart** in the CE Health tab (`render_ce_health.py` `_FAC`/`_FLBL`/`_facs` recast 6→5-factor — this was the gap: the chart stakeholders read still had OPC + converter-based CVR while the preview had been changed), **(3)** the Summary tab driver table (inherits §7's factors; caption updated). Scope: this CVR (orders/users) lives **only inside the Shapley** — the §2 vitals CVR card, the funnel section, and the `cvr`/`users` sidecar keep converters/users (Omni); each Shapley surface gets a one-line caption disclosing the difference. Verified: both files parse; §7 render + engine both reconcile (`unattributable = $0`) on CE 2567, no `orders_per_converter`, CVR = orders/users; zero live "6-factor" / "Orders / Converter" left. Blast radius: `skills/ce-health/ce_health.py`, `scripts/render_ce_health.py`, `SKILL.md` (Step-1 caption + §7 wording + this row), `references/summary_guide.md`, `CHANGELOG.md`, `VERSION`. No `compose.py` / template / CVR-RCA / perf-audit / contract change; vitals + funnel untouched. |
| m082 | 2026-06-16 | **Composite tab-switch: relayout hidden-pane Plotly charts to real width (fixes CVR trend chart stuck at 700px) (v2.47.0).** A live run (CE 2174) rendered the CVR RCA tab with its daily-CVR-trend chart (and surrounding top section) sized wrong — the chart's `_fullLayout.width` was pinned at Plotly's **700px width-0 fallback**. Root cause (verified live via preview-DOM inspection): the composite's default tab is **Summary**, so the CVR pane loads `display:none`; the CVR report's Plotly chart runs `newPlot` on load inside that hidden pane → 0 container width → Plotly's 700px fallback baked into `_fullLayout.width`. The composite's `activateTab` (in `templates/report.html`) tried to recover with **`Plotly.Plots.resize(el)`**, which is a **verified no-op** for this case (chart stayed 700px even with the pane visible and `clientWidth` 952). This is composite-specific: standalone CVR-RCA has CVR as the default *visible* tab, so the chart measures correctly and never hits the fallback. Fix (in the composite tab framework, owned by ce-rca): replace `Plotly.Plots.resize` with an explicit **`Plotly.relayout(el, {width: el.clientWidth, autosize:true})`** (verified to reflow 700→952), deferred via **double `requestAnimationFrame` + a 150ms `setTimeout` fallback** (the single-rAF version raced the chart's initial `newPlot` and left it at 700), guarded by `clientWidth > 0`. Generalises to every non-default tab's charts (e.g. CE Health). Verified end-to-end in a headless browser: fresh load → click CVR → chart SVG = 952px = container width. **Secondary (NOT fixed, flagged):** CVR-RCA's report emits a **duplicate `id="trend-90day"`** on both the `.section` wrapper and the inner `.chart-container`, so Plotly renders into the section wrapper (wiping its `<h3>`); harmless in standalone, cosmetic in composite — a CVR-RCA authoring bug to fix upstream. Blast radius: `templates/report.html` (activateTab) + this row, `CHANGELOG.md`, `VERSION`. No engine / renderer / `compose.py` / CVR-RCA / perf-audit change. |
| m081 | 2026-06-16 | **Orchestrator guardrail — verify perf-audit's Step 4b data dump landed; re-prompt if §B is empty (v2.46.0).** A live run (CE 2174) shipped a perf-audit tab whose narrative pointed to "full dump in Sheet Tab 6/7/8" but **no Sheet existed** — §B (Data Sources) was an empty heading. Root cause was a pure **execution skip**, not a missing/contradicting instruction: perf-audit's **Step 4b** (create the 8-tab data-dump Sheet, append the link to §B) sits *after* the report narrative reads as finished, is labeled a sub-step ("4b"), produces an *external* side-artifact, and is *forward-referenced* by the Step-3 narrative — so a sub-agent that treats the written report as "done" skips it, leaving dangling Tab pointers every time. Fix is an **orchestrator-side forcing function** (kept out of upstream-owned perf-audit): **(1)** the Step-2 perf-audit dispatch prompt now names Step 4b **mandatory** — *"the run is NOT complete until §B contains the data-dump Sheet link or the local-CSV fallback paths; do not stop at the written report"* (always satisfiable → never blocks). **(2)** a new **post-dispatch gate** `awk`-greps §B of `perf_audit_report.md` for a Sheet link / `.csv` / `docs.google.com` / `Tab 1:` marker; on `MISSING`, **re-prompts the same perf-audit agent via `SendMessage`** (its context is intact) to run Step 4b only, re-checks, and proceeds. Graceful: if it still can't produce after one re-prompt (no Drive access + CSV export fails), note + continue, pointers stay dangling but run isn't blocked. Verify-don't-trust — perf-audit stays the owner; ce-rca only checks the artifact it already post-processes. Blast radius: `SKILL.md` Step 2 (dispatch bullet + post-dispatch gate + this row), `CHANGELOG.md`, `VERSION`. No engine / renderer / `compose.py` / perf-audit / CVR-RCA change. |
